...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.data_stamp is not None:
            # 全量注入
            if self.data_stamp.shape[0] > data.shape[0]:
                time_n, channel = self.data_stamp.shape
                self.data_stamp = self.data_stamp.reshape(33, time_n // 33, channel)
                # self.data_stamp = self.data_stamp.reshape(64, time_n // 64, channel)
                self.data_stamp = self.data_stamp.permute(1, 2, 0)
    ...
    def __getitem__(self, index):
        ...
        if self.data_stamp is not None:
            ...
            return seq_x, seq_y, torch.zeros(1, 4096), torch.zeros(1, 4096)
        # seq_x_mark = self.data_stamp[s_begin:s_end:self.token_len]
        # seq_y_mark = self.data_stamp[s_end:r_end:self.token_len]
        #
        # return seq_x, seq_y, seq_x_mark, seq_y_mark
...
class Dataset_ETT_minute(Dataset):
    def __init__(
        ...
        assert size is not None and len(size) == 3, "size 必须是 [seq_len, label_len, pred_len]"
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]

        self.token_len = self.seq_len - self.label_len
        assert self.token_len > 0, "token_len=seq_len-label_len 必须 > 0"
        self.token_num = self.seq_len // self.token_len
        ...
        self.enc_in = self.data_x.shape[-1]
        self.tot_len = len(self.data_x) - self.seq_len - self.pred_len + 1
        assert self.tot_len > 0, "滑窗数量 <= 0，请检查 size 设置与数据长度"
    ...
    def __read_data__(self):
        ...
        data_name = self.data_path.split('.')[0]
        pt_path = os.path.join(self.root_path, f'{data_name}.pt')
        self.data_stamp = None
        if os.path.exists(pt_path):
            try:
                stamp = torch.load(pt_path)
                if stamp.shape[0] > data.shape[0]:
                    time_n, channel = stamp.shape
                    stamp = stamp.reshape(33, time_n // 33, channel)
                    stamp = stamp.permute(1, 2, 0)
        ...
        self.data_x = data[border1:border2]  # [T, C]
        self.data_y = data[border1:border2]  # [T, C]
    ...
    def __getitem__(self, index):
        ...
        if self.data_stamp is not None:
            ds = self.data_stamp
            if ds.ndim == 2:
                seq_x_mark = ds[s_begin:s_end:self.token_len]
                seq_y_mark = ds[s_end:r_end:self.token_len]
            elif ds.ndim == 3:
                seq_x_mark = ds[s_begin:s_end:self.token_len, ...]
                seq_y_mark = ds[s_end:r_end:self.token_len, ...]
            else:
                seq_x_mark = torch.zeros(1, 4096)
                seq_y_mark = torch.zeros(1, 4096)
...
class Dataset_Preprocess_TSF(Dataset):
    def __init__(self, data_root):
        self.train_data = np.load(data_root)
        f = open("etth1_str.txt", 'r', encoding="utf-8")
        f_lines = f.readlines()
        self.f_lines = [item.strip("\n") for item in f_lines]

    def __getitem__(self, index):
        # X = list(self.train_data[index, :672])
        # Y = list(self.train_data[index, 672:])
        X = list(self.train_data[index, :48])
        Y = list(self.train_data[index, 48:96])
        X = [str(round(item, 2)).zfill(2) for item in X]
        Y = [str(round(item, 2)).zfill(2) for item in Y]
        X = ",".join(X)
        Y = ",".join(Y)
        return_str = (self.f_lines[index] +
                      (". The input time series is {}. "
                                            "We need to predict the time series: {}").
                      format(X, Y))
        return return_str
