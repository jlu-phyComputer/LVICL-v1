...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                task_vector = np.load(self.args.root_path + "task_v_ICL.npy")
                batch_x_mark = torch.from_numpy(task_vector).unsqueeze(0).unsqueeze(0)
                ...
                show_num = 100
                if "ETT" not in self.args.root_path:
                    show_num = 1000
                if (i + 1) % show_num == 0:
                    if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    loss_val += loss.item()
                    count += 1
                if "nan" in str(loss.item()):
                    break_flag = True
                    print("break_flag = True")
                show_num = 100
                if "ETT" not in self.args.root_path:
                    show_num = 1000
                if (i + 1) % show_num == 0:
                    if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                        print_str = "\titers: {0}, epoch: {1} | loss: {2:.7f}".format(i + 1, epoch + 1,
                                                                                      loss_val.item()/count)
                        # print_str = "\titers: {0}, epoch: {1} | loss: {2:.7f}".format(i + 1, epoch + 1,
                        #                                                               loss.item())
                        print(print_str)
    ...
    def test(self, setting, test=0, test_pred_len=96, output_hidden_states=False):
        print("test_pred_len=", test_pred_len)
        self.args.test_pred_len = test_pred_len
        test_data, test_loader = self._get_data(flag='test')
        ...
        if test:
            ...
            # load_item = torch.load("checkpoints/ETTh1_best/checkpoint.pth")
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                for j in range(inference_steps):
                    ...
                    if self.args.use_amp:
                        with torch.cuda.amp.autocast():
                            outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, output_hidden_states)
                    else:
                        outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, output_hidden_states)
                ...
                if output_hidden_states:
                    if len(hidden_states_list) == 0:
                        for k in range(len(hidden_states)):
                            hidden_states_list.append(hidden_states[k].unsqueeze(0))
                    else:
                        for k in range(len(hidden_states)):
                            hidden_states_list[k] = torch.cat([hidden_states_list[k], hidden_states[k].unsqueeze(0)])
        ...
        if output_hidden_states:
            hidden_states_list = [item.mean(0).unsqueeze(-1) for item in hidden_states_list]
            hidden_states_list = torch.cat(hidden_states_list, dim=-1)
            np.save("temp.npy", hidden_states_list.detach().cpu().numpy())
