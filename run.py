...
def main(lr=0.0005, init_beta=1):
    fix_seed = 2021
    random.seed(fix_seed)
    torch.manual_seed(fix_seed)
    np.random.seed(fix_seed)

    parser = argparse.ArgumentParser(description='ICL4LLMTSF')
    ...
    parser.add_argument('--model_id', type=str, required=False, default='ETTh1_672_96', help='model id')
    parser.add_argument('--model', type=str, required=False, default='LVICL_Llama',
                        help='model name, options: [LVICL_Llama, LVICL_Gpt2, LVICL_Opt1b]')
    ...
    parser.add_argument('--init_beta', type=float, default=init_beta, help='dropout')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if not break_flag:
                if args.task_name == 'long_term_forecast':
                    exp.test(setting, test=1, test_pred_len=96)
                    torch.cuda.empty_cache()
                    exp.test(setting, test=1, test_pred_len=192)
                    torch.cuda.empty_cache()
                    exp.test(setting, test=1, test_pred_len=336)
                    torch.cuda.empty_cache()
                    exp.test(setting, test=1, test_pred_len=720)
                    torch.cuda.empty_cache()
                else:
                    exp.test(setting, test=1, test_pred_len=args.test_pred_len)
                    torch.cuda.empty_cache()
    else:
        ii = 0
        setting = '{}_{}_{}_{}_sl{}_ll{}_tl{}_lr{}_bt{}_wd{}_hd{}_hl{}_cos{}_mix{}_{}_{}'.format(
        ...
        if args.task_name == 'long_term_forecast':
            exp.test(setting, test=1, test_pred_len=96, output_hidden_states=False)
            torch.cuda.empty_cache()
            exp.test(setting, test=1, test_pred_len=192)
            torch.cuda.empty_cache()
            exp.test(setting, test=1, test_pred_len=336)
